...
本章将逐步讨论适用于所有集合类型的常见操作，然后讨论各个数据结构以及使用场景中使用它们。
...
在这些集合转换中涉及的复制确实有一些开销，但在大多数情况下不会引起问题。如果某段代码确实是导致程序运行缓慢的瓶颈，你可以随时转换`.map`,`.filter`等，将代码转换为对原始数组的变异运算，以优化性能，或使用可变集合和就地操作(4.3.4)
...
再举一个例子，使用的函数`.exists`，`.map`和 `.distinct`检查输入数字网格是否是有效的数独网格:
...
数独网格为例，用二维数组`Array[Array[Int]]`表示。对于每一个从0到9的i，我们挑选出一个单独的行、列和3x3方块。然后，它通过调用`.deparate来`检查每个这样的行/列/方格是否有9个唯一的数字，以去除任何重复的数字，然后检查`.lengt`h是否因为去除而改变了。。
...
一般来说，当你知道一个序列不会改变，但需要灵活处理的时候，使用`Vectors`是很方便的，它们的树结构使大多数操作相当高效，尽管它们永远也不会像`Arrays`（用于就地更新）或`Immutable Lists` (4.2.5)那样在前面添加和删除元素的速度快
...
大多数不可变的集合操作在集合的大小时的时间复杂度为$O(log n)$。这对于大多数目的来说已经足够快了，但是在不快的情况下，你总是可以使用可变` Sets` (4.3.2)来获得更好的性能。` Sets`也支持所有集合通用的标准操作。
...
Scala标准库提供了可变`Sets`，作为我们前面看到的不可变`Sets`的对应，可变集合也提供了有效的`.contains`检查$(O(1))$，但是，不是通过`+`和`-`构造集合的新副本，而是通过`.add`和`.remove`从集合中添加和删除元素:
...
本章为您提供了Scala集合库的基础知识，该Scala集合库在每个Scala程序中都被广泛使用。现在，我们将讨论Scala语言的一些更独特的特性，以完成对Scala的介绍。
